"""Robust Discovery module with fallback methods for finding UN reports."""
...
class RobustUNDLClient:
    """Robust client with multiple fallback methods for finding UN reports."""
    ...
    def search_reports_robust(self, query: str = "reports 2025", max_results: int = 50) -> List[Dict[str, Any]]:
        """Search for UN reports using multiple methods with fallbacks."""
        logger.info(f"Starting robust search for: {query}")
        
        # Method 1: Try the official UNDL API
        reports = self._try_official_api(query, max_results)
        if reports:
            logger.info(f"Official API found {len(reports)} reports")
            return reports
        
        # Method 2: Try the search endpoint directly
        reports = self._try_search_endpoint(query, max_results)
        if reports:
            logger.info(f"Search endpoint found {len(reports)} reports")
            return reports
        
        # Method 3: Try to find reports by known patterns
        reports = self._try_known_patterns(query, max_results)
        if reports:
            logger.info(f"Known patterns found {len(reports)} reports")
            return reports
        
        logger.warning("All search methods failed")
        return []
    
    def _try_official_api(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        """Try the official UNDL API."""
        try:
            ...
            if result and 'records' in result:
                ...
                return valid_reports[:max_results]
    ...
    def _try_search_endpoint(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        """Try the search endpoint directly."""
        try:
            params = {
                "p": query,
                "ln": "en",
                "rg": min(max_results, 200)
            }
            ...
            # Parse the HTML response to extract record IDs
            record_ids = self._extract_record_ids_from_html(response.text)
            ...
            if record_ids:
                ...
                for record_id in record_ids[:max_results]:
                    try:
                        report = self.get_record_by_id(record_id)
                        if report:
                            reports.append(report)
                        time.sleep(1)  # Rate limiting
                    except Exception as e:
                        logger.debug(f"Failed to get record {record_id}: {e}")
                        continue
    ...
    def _extract_record_ids_from_html(self, html_content: str) -> List[str]:
        """Extract record IDs from search results HTML."""
        record_ids = []
        
        # Look for record links in the HTML
        import re
        pattern = r'/record/(\d+)'
        ...
        for match in matches:
            if match not in seen:
                seen.add(match)
                record_ids.append(match)
        
        return record_ids
    
    def _try_known_patterns(self, query: str, max_results: int) -> List[Dict[str, Any]]:
        """Try to find reports using known patterns and recent record IDs."""
        reports = []
        
        # Try some known recent record IDs
        known_ids = [
            "4084949",  # UNCTAD TIR 2025
            "4084950",  # UNCTAD TIR 2025 Overview
            "4087110",  # JIU Report 2025
            "4085123",  # Another recent document
        ]
        
        for record_id in known_ids[:max_results]:
            try:
                report = self.get_record_by_id(record_id)
                if report and self._matches_query(report, query):
                    reports.append(report)
                time.sleep(1)  # Rate limiting
            except Exception as e:
                logger.debug(f"Failed to get known record {record_id}: {e}")
                continue
    ...
    def _matches_query(self, report: Dict[str, Any], query: str) -> bool:
        ...
        symbol = report.get('symbol', '').lower()
        
        # Check if query terms appear in title or symbol
        query_terms = query_lower.split()
        for term in query_terms:
            if term in title or term in symbol:
                return True
...
class RobustUNReportDiscoverer:
    """Robust discoverer with multiple fallback methods."""
    
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.client = RobustUNDLClient(config)
        self.start_date, self.end_date = get_date_window(config['date_window_days'])
        
    def discover_all(self) -> List[Dict[str, Any]]:
        """Main discovery method using robust search."""
        logger.info("Starting robust UN reports discovery...")
        ...
        for query in queries:
            try:
                reports = self.client.search_reports_robust(query, max_results=20)
