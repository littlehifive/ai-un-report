...
class UNReportFetcher:
    ...
    def fetch_from_records(self, records_file: str) -> Dict[str, Any]:
        ...
        for idx, record in records_df.iterrows():
            ...
            # Download first available file URL
            for url in file_urls:
                if pd.isna(url) or not url.strip():
                    continue
                    
                result = self.download_file(url, symbol, language)
                download_results.append(result)
                
                if result['status'] == 'success':
                    successful_downloads += 1
                    break  # Successfully downloaded, no need to try other URLs
                else:
                    failed_downloads += 1
                    
            else:
                # No successful download for this record
                logger.warning(f"All download attempts failed for {symbol}")
